#######Bài tập Chương 2#######
#BT1: Sử dụng hàm c, tạo một biến chứa các giá trị độ dài của bảy con vật.
#Đồng thời tạo một biến chứa các giá trị Tb (bao gồm cả NAs). 
#Chiều dài trung bình của bảy con vật là bao nhiêu?
LengthCT <- c(75, 85, 91.6, 95, NA, 105.5, 106)
Tb <- c(0, 0, 1, NA, 0, 0, 0)
mean(LengthCT, na.rm = TRUE)
#BT2: Sử dụng lệnh cbind để kết hợp dữ liệu tháng, độ dài và Tb, và lưu trữ kết quả trong biến Boar. 
...
ncol(Boar)
Boar
#BT3: Chúng ta tiếp tục với hươu từ bài 1. 
#Thay vì hàm c mà bạn đã sử dụng trong bài 2 để kết hợp dữ liệu Tb. 
#Bạn có thể làm tương tự với hàm vectơ không? Đặt tên khác cho vectơ, chẳng hạn như Tb2.
Tb2 <- vector(length = 7)
Tb2[1] = 0
Tb2[2] = 0
Tb2[3] = 1
...
##bội với nghịch 
D%*%solve(D)
#BT 5: Hãy tạo một data frame chứa tất cả các dữ liệu được trình bày trong bảng ở bài 1. 
#Giả sử rằng bạn quyết định bình phương dữ liệu độ dài. Thêm dữ liệu đó vào data frame. 
#Làm bài tập tương tự với list thay vì data.frame.
df <- data.frame(
  Farm,
  Month,
...
df
#BT 6: Nhập dữ liệu vào R trước tiên bằng cách sử dụng hàm read.table và sau đó là hàm scan.
#Sử dụng hai tên khác nhau để lưu trữ dữ liệu. 
#Sự khác biệt giữa hai hàm là gì? Sử dụng các hàm is.matrix và is.data.frame để trả lời câu hỏi này.
library(readxl)
...
Deerascii
#########Bài tập chương 3########
#BT1: Tệp BirdFlu.xls chứa số trường hợp xác nhận hàng năm về cúm gia cầm A(H5N1) ở người từ một số quốc gia đã báo cáo cho tổ chức y tế thế giới (WHO). 
#In số ca mắc cúm gia cầm năm 2003.Tổng số ca mắc cúm gia cầm năm 2003 và năm 2005 là bao nhiêu? 
#Quốc gia nào đã có nhiều trường hợp nhất? Quốc gia nào có ít ca tử vong do cúm gia cầm nhất? 
#Cho biết tổng số ca mắc cúm gia cầm của mỗi quốc gia là bao nhiêu? Tổng số vụ mỗi năm là bao nhiêu?
...
df
#BT2: #Tệp ISIT.xls. Trong R, trích xuất dữ liệu từ trạm 1. Có bao nhiêu quan sát được tại trạm này? 
...
nrow(ISIT.Station1)
# min, median, mean, max độ sâu lấy mẫu
# Station 1
...
#BT3: Trong bước cuối cùng của bài tập trước, dữ liệu đo được ở độ sâu hơn 2000 mét vào tháng 4 đã được trích xuất. 
...
############Bài tập chương 4###########
#BT1: Tính toán giá trị trung bình hàng tháng một lần bằng cách sử dụng dữ liệu từ tất cả các trạm.
#Kết quả cuối cùng phải là một ma trận 16 x 12. 
#Đồng thời tính độ lệch chuẩn và số lần quan sát mỗi tháng
...
count
#BT4: Sử dụng dữ liệu trong bài 1, xác định số lần quan sát trên mỗi trạm.
#Có bao nhiêu quan sát được thực hiện mỗi năm? Có bao nhiêu quan sát được thực hiện ở mỗi trạm mỗi năm?
...
#######Bài tập chương 5###########
#BT1:Tạo một plot của TOT_N so với D_park. 
#Sử dụng nhãn thích hợp.Thêm một đường cong làm mịn. 
#Tạo lại plot nhưng sử dụng điểm tỷ lệ với giá trị của OLIVE (điều này có thể cho biết liệu OLIVE có ảnh hưởng hay không).
...
#BT2: Sử dụng ifelse tạo một biến phân loại mới xác định những quan sát từ chế độ ăn tại một tổ cụ thể. 
#Thử chạy lại mã từ bài 1 để lập biểu đồ thương lượng giữa anh chị em với thời gian đến cùng một tổ và và phân loại chế độ ăn.
...
boxplot(Veg$R)
#BT4: Tạo một ô vuông của số lượng ký sinh trùng (Cường độ) tùy theo khu vực, giới tính, giai đoạn hoặc năm. 
#Thử các kết hợp để phát hiện các tương tác.
...
#Lập biểu đồ chấm hiển thị thời gian đến nơi mỗi đêm. 
#Và phân biệt chế độ ăn của của mỗi đêm.
